...
#fix stressCtl NetworkMon addforce v_FperX v_FperY 0.0
#fix defY NetworkMon deform 1 y erate v_erateY units box remap x
#fix defX NetworkMon deform 1 x erate v_erateX units box remap x

fix		fix_print all print ${thermodump} "${step} ${etot} ${ke} ${peBond} ${peAngle} ${temp} ${press} ${pxx} ${pyy} ${StressX} ${StressY} ${StressZ} ${StressX_N} ${StressY_N} ${StressZ_N} ${sigma_xxNet} ${sigma_yyNet} ${FtotX} ${FtotY} ${Fbond} ${NumType9} ${box_lengthX} ${box_Vol} ${CurrentConc} ${conc_preferred} ${P_add} ${P_remov} ${NumNuc} ${NumDeath} ${TermDen_add} ${TermDen_remov} ${TermId_add} ${TermId_remov} ${NumTot}" file ${thermofile} screen no title "step etot ke peBond peAngle temp press pxx pyy stressX stressY stressZ stressX_N stressY_N stressZ_N sigma_yy sigma_xx FtotX FtotY AvBondForce NumMons BoxLength BoxVol CurrentConc ConcPref P_add P_remov NreactNuc NreactDeath T_den_add T_den_remov T_id_add T_id_remov NumTot"
