...
def main():
    ...
    Press = float(sys.argv[21])
    Pdamp = str(sys.argv[22])
    seed  = str(sys.argv[23])
    Nstretch = 20
    tmix = 0 #1e5
    #tAssemble = 1e6
...
''')
    ...
    fo.write('variable k_nc1			equal	1.0 		#Elastic constant A type bond\n')
    fo.write('variable r0_nc1			equal	0.5 		#Rest length A type bond\n\n')

    fo.write('variable k_7s			equal	1.0 		#Elastic constant B type bond\n')
...
group PureMon type 9 2
group NetworkMon type 1 3 4 5 6 7 8
...
compute 		pressComp all pressure thermo_temp
variable pxx equal c_pressComp[1]
variable pyy equal c_pressComp[2]

compute perAtomStress all stress/atom NULL
compute perAtomStressN NetworkMon stress/atom NULL
...
compute totalStressXNet NetworkMon reduce sum c_perAtomStressN[1]
compute totalStressYNet NetworkMon reduce sum c_perAtomStressN[2]
compute totalStressZNet NetworkMon reduce sum c_perAtomStressN[3]
compute forceBond all bond/local force
#compute stNet NetworkMon stress/atom NULL
compute forceBondAll all reduce ave c_forceBond

#compute pNet NetworkMon pressure NULL c_perAtomStressN

''')
    fo.write('variable PressTarget equal	'+str(Press)+'\n')
    fo.write('''
...
variable StressX_N equal c_totalStressXNet
variable StressY_N equal c_totalStressYNet
variable StressZ_N equal c_totalStressZNet
variable Fbond equal c_forceBondAll


# 1. Compute network stress
variable sigma_yyNet equal -c_totalStressYNet/vol
variable sigma_xxNet equal -c_totalStressXNet/vol
variable gain equal 0.3
variable areaY equal vol/ly
variable areaX equal vol/lx
variable Nnet equal count(NetworkMon)

# Stress error (what we have minus what we want)
variable errY equal v_sigma_yyNet-v_PressTarget
variable errX equal v_sigma_xxNet-v_PressTarget


# Strain rate for Ly: erate has units 1/time
# Negative sign: if σ > target, we shrink box (negative erate),
# if σ < target, we expand it (positive erate)
variable erateY     equal -v_gain*v_errY
variable erateX     equal -v_gain*v_errX

# Total force in y needed to correct stress:
# F_tot ≈ - gain * (σ_yyNet - σ_target) * Area_y
variable FtotY equal -v_gain*v_errY*v_areaY
variable FtotX equal -v_gain*v_errX*v_areaX

# Per-atom force in y to apply to each network atom
variable FperY equal v_FtotY/v_Nnet
variable FperX equal v_FtotX/v_Nnet
...
variable conc_pref_tot equal 721/v_volInit
...
fix             freact_creation all bond/react stabilization no reset_mol_ids no &
                react Nucleation all ${NeveryFast} ${RminF} ${RmaxFB} nucleation_pre nucleation_post ../../templates/Nucleation/map_Nucleation.txt prob 1 ${lseed1} modify_create nuc yes &
                react Death all ${NeveryFast} ${RminF} ${RmaxFB} death_pre death_post ../../templates/Nucleation/map_Death.txt prob 1 ${lseed1} &
...
variable            NumRelNC1 equal 2*f_freact_creation[3]+f_freact_creation[4]+f_freact_creation[5]+f_freact_creation[6]+f_freact_creation[7]+f_freact_creation[8]+f_freact_creation[9]+f_freact_creation[10]+f_freact_creation[11]+f_freact_creation[12]
variable            NumRel7S_1 equal 2*f_freact_creation[13]+f_freact_creation[14]+f_freact_creation[15]+f_freact_creation[16]+f_freact_creation[17]+f_freact_creation[18]+f_freact_creation[19]
variable            NumRel7S_2 equal f_freact_creation[20]+f_freact_creation[21]+f_freact_creation[22]+f_freact_creation[23]+f_freact_creation[24]+f_freact_creation[25]+f_freact_creation[26]+f_freact_creation[27]+f_freact_creation[28]+f_freact_creation[29]
variable            NumRelease equal v_NumRelNC1+v_NumRel7S_1+v_NumRel7S_2
variable            NumIncNC1 equal 2*f_freact_creation[30]+f_freact_creation[31]+f_freact_creation[32]+f_freact_creation[33]+f_freact_creation[34]+f_freact_creation[35]+f_freact_creation[36]+f_freact_creation[37]+f_freact_creation[38]+f_freact_creation[39]
variable            NumInc7S_1 equal 2*f_freact_creation[40]+f_freact_creation[41]+f_freact_creation[42]+f_freact_creation[43]+f_freact_creation[44]+f_freact_creation[45]+f_freact_creation[46]
variable            NumInc7S_2 equal f_freact_creation[56]+f_freact_creation[47]+f_freact_creation[48]+f_freact_creation[49]+f_freact_creation[50]+f_freact_creation[51]+f_freact_creation[52]+f_freact_creation[53]+f_freact_creation[54]+f_freact_creation[55]
variable            NumIncorp equal v_NumIncNC1+v_NumInc7S_1+v_NumInc7S_2


fix		fix_print all print ${thermodump} "${step} ${etot} ${ke} ${peBond} ${peAngle} ${temp} ${press} ${pxx} ${pyy} ${StressX} ${StressY} ${StressZ} ${StressX_N} ${StressY_N} ${StressZ_N} ${sigma_xxNet} ${sigma_yyNet} ${FtotX} ${FtotY} ${Fbond} ${NumType9} ${box_lengthX} ${box_Vol} ${CurrentConc} ${conc_preferred} ${P_add} ${P_remov} ${NumNuc} ${NumDeath} ${TermDen_add} ${TermDen_remov} ${TermId_add} ${TermId_remov} ${NumTot} ${NumRelease} ${NumIncorp}" file ${thermofile} screen no title "step etot ke peBond peAngle temp press pxx pyy stressX stressY stressZ stressX_N stressY_N stressZ_N sigma_yy sigma_xx FtotX FtotY AvBondForce NumMons BoxLength BoxVol CurrentConc ConcPref P_add P_remov NreactNuc NreactDeath T_den_add T_den_remov T_id_add T_id_remov NumTot NumRelease NumIncorp"


''')
    fo.write('fix                    fDeformY all press/berendsen y '+str(Press)+' '+str(Press)+' '+str(Pdamp)+'\n')
    fo.write('fix                    fDeformX all press/berendsen x '+str(Press)+' '+str(Press)+' '+str(Pdamp)+'\n')
    fo.write('''
