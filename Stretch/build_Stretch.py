...
def main():
    ...
    XStretch = float(sys.argv[21])
    seed  = str(sys.argv[22])
    Nstretch = 20
    tmix = 1e4
    #tAssemble = 1e6
...
''')
   ...
   # fo.write('variable tassemble       equal 	'+str(tAssemble)+'	#NVT steps for bond equilibration\n')
    ...
    fo.write('variable tInitRel			equal 	'+str(int(float(tStr)/2))+'	#NVT steps for bond equilibration\n')  #usually * 20 when stretch = 1e4
...
variable conc_pref_tot equal 3125/v_volInit #Half density : 1621 34 density : 2421
...
fix		fix_print all print ${thermodump} "${step} ${etot} ${ke} ${peBond} ${peAngle} ${temp} ${press} ${StressX} ${StressY} ${StressZ} ${StressX_N} ${StressY_N} ${StressZ_N} ${Fbond} ${NumType9} ${box_lengthX} ${box_Vol} ${CurrentConc} ${conc_preferred} ${P_add} ${P_remov} ${NumNuc} ${NumDeath} ${TermDen_add} ${TermDen_remov} ${TermId_add} ${TermId_remov} ${NumTot}" file ${thermofile} screen no title "step etot ke peBond peAngle temp press stressX stressY stressZ stressX_N stressY_N stressZ_N AvBondForce NumMons BoxLength BoxVol CurrentConc ConcPref P_add P_remov NreactNuc NreactDeath T_den_add T_den_remov T_id_add T_id_remov NumTot"
...
''')
    for i in range(Nstretch):
        ...
        fo.write('run                    ${tstretchcycl}\n') # every 10850 "write_data snapshot_*.data"\n')
